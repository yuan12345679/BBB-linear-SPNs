...
%
Formally, an $r$-round SPN taking inputs of length $w n$ is defined by $r+1$ round keys $k_0,k_1,\ldots,k_r\in\{0,1\}^{wn}$, $r$ permutations $S_1,\ldots,S_r:\{0,1\}^{n} \rightarrow \{0,1\}^{n}$, and an invertible linear permutation $T\in\mathbb{F}^{w\times w}$. Given an input $x \in \{0,1\}^{w n}$, the output of the SPN is computed as follows:
