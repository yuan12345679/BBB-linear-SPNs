...
%
\begin{align}
\operatorname{Adv}_{\spn^T}^{\mathrm{su}}(p, q) \leq& \frac{w^3q^3}{N^2}-
\frac{6w^2 q (p+wq)^2}{N^{2}}-
\frac{4w^4q^2(p+wq)}{N^2}-\frac{q^2}{N^w}-\frac{q(2wp+6w^2q)^2}{N^2}  .  
\notag   \\
...
We will rely on the following lemma, which characterizes some useful properties of the $t$-th round of the linear SPN.

\begin{lemma}
	\label{lemma:coll-prob}
	
	For any $t\in\{1,2,3,4\}$, $z,z',\delta\in\{0,1\}^n$, and $i,i',j,j'\in\{1,\ldots,w\}$, define
	\begin{align*}
	...
	&\midindent~\Big|~\neg\bone\wedge\neg\btwo\wedge S_t\vdash\mathcal{Q}_{S_t}^{(0)}\wedge\forall\ell\in\{1,\ldots,w\}:(z\xor k_{t-1})[\ell]\notin U_t^{(0)}\Big],         \\
	%
	&\pcoll_{1}^-(t,z,z',j,j')     :={\Pr}\Big[\big(T^{-1}\big(\overline{S_t^{-1}}(z\xor k_t)\big)\xor k_{t-1}\big)[j]=\big(T^{-1}\big(\overline{S_t^{-1}}(z'\xor k_t)\big)\xor k_{t-1}\big)[j']    \\
	&\midindent~\Big|~\neg\bone\wedge\neg\btwo\wedge S_t\vdash\mathcal{Q}_{S_t}^{(0)}\wedge\forall\ell\in\{1,\ldots,w\}:(z\xor k_t)[\ell]\notin V_t^{(0)}\Big],         \\
	&\pcoll_{2}^-(t,z,z',i,i',j,j')    :={\Pr}\Big[\big(T^{-1}\big(\overline{S_t^{-1}}(z\xor k_t)\big)\xor k_{t-1}\big)[j]=\big(T^{-1}\big(\overline{S_t^{-1}}(z'\xor k_t)\big)\xor k_{t-1}\big)[j']     \\
	&\midindent~\Big|~\neg\bone\wedge\neg\btwo\wedge S_t\vdash\mathcal{Q}_{S_t}^{(0)}\wedge(z\xor k_t)[i]\in V_t^{(0)}\wedge(z'\xor k_t)[i']\in V_t^{(0)}\Big],         \\
	%
	&\pcoll_{3}^-(t,z,i,\delta)      :={\Pr}\Big[\big(T^{-1}\big(\overline{S_t^{-1}}(z\xor k_t)\big)\xor k_{t-1}\big)[i]=\delta    \\
	&\midindent~\Big|~\neg\bone\wedge\neg\btwo\wedge S_t\vdash\mathcal{Q}_{S_t}^{(0)}\wedge\forall\ell\in\{1,\ldots,w\}:(z\xor k_t)[\ell]\notin V_t^{(0)}\Big],
	\end{align*}
	where the probabilities are taken over the random choices of $S_t$, $k_{t-1}$, and $k_t$. Then, as long as $(z,j)\neq(z',j')$, it holds
	\begin{align}
...
\begin{proof}
	...
	By the above, in any case, the probability to have $T(\overline{S_t}(z\xor k_{t-1}))[j]=T(\overline{S_t}(z'\xor k_{t-1}))[j]$ is at most $1/(N-p-wq)$, which establishes $\pcoll_{1}^+(t,z,z',j,j')\leq1/(N-p-wq)$. Similarly by symmetry, $\pcoll_{1}^-(t,z,z',j,j')\leq1/(N-p-wq)$.
...
%For (B-43), we have
For (B-42), for each $((x',a',b',y'),j')$ such that $(x,j)\neq(x',j')$, $(x'\xor k_0)[i']\notin U_1^{(0)}$ for all $i'\in\{1,\ldots,w\}$, we have $\pcoll_{1}^+(1,x',x,j',j)\leq1/(N-p-wq)$ by Lemma \ref{lemma:coll-prob}. Thus $\Pr[\text{(B-42)}]\leq wq/(N-p-wq)$.
...
For (B-43), for each $((x',a',b',y'),i',j')$ such that $(x,j)\neq(x',j')$, the probability to have $(a\xor k_1)[j]=(a'\xor k_1)[j']$ never exceeds $1/(N-p-wq)$:
%
\begin{itemize}
	\item If there exists $i\in\{1,\ldots,w\}$ such that $(x\xor k_0)[i]\in U_1^{(0)}$, then by Lemma \ref{lemma:coll-prob}, we have $\pcoll_{2}^+(1,x,x',i,i',j,j')\leq1/(N-p-wq)$;
	\item Else, we have $\pcoll_{1}^+(1,x,x',j,j')\leq1/(N-p-wq)$ by Lemma \ref{lemma:coll-prob}.
\end{itemize}
%
Therefore, $\Pr[\text{(B-43)}]\leq w^2q/(N-p-wq)$.
...
However, note that the subconditions (B-42) and (B-43) are mutual exclusive. Summing over the above and using this mutual exclusiveness and $p+wq\leq N/2$, we reach
%
$${\Pr}\big[(a\xor k_1)[i]\in U_2^{(1)}\big]\leq
\frac{p}{N}+\frac{wq}{(N-p-wq)}\leq\frac{p+wq}{N}.$$
%

Similarly (one round further),
%
$${\Pr}\big[(T(\overline{S_2}(a \oplus k_1))\xor k_2)[j]\in U_{3}^{(1)}\big]\leq\frac{p+wq}{N},$$
%
and similarly by symmetry,
%
\begin{align*}
&{\Pr}\big[(b\xor T^{-1}(k_3))[j]\in V_3^{(1)}\big]\leq\frac{p+wq}{N},       \\
&{\Pr}\big[(T^{-1}(\overline{S_3^{-1}}(b\xor T^{-1}(k_3))\xor k_2))[i]\in V_2^{(1)}\big]\leq\frac{p+wq}{N}.
\end{align*}
%
By this, the probability that \bfour is fulfilled with respect to each choice of $((x,a,b,y),i, j)$ is at most $3(p+wq)^2/N^2$. As there are at most $w^2q$ such choices, we eventually obtain
%
$$
{\Pr}\big[\bfour\big] \leq \frac{3w^2q(p+wq)^2}{N^2}.
$$
...
\begin{align*}
\Pr[\bfive] 
=   &  \sum_{(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'}\sum_{i,i',j,j'}\bigg(\underbrace{{\Pr}\big[(a\xor k_1)[i]\in U_2^{(1)}\big]}_{\leq(p+wq)/(N-p-wq)\text{, as argued before}}     \\
& \midindent\times
...
%
\begin{align*}
&  {\Pr}\big[ \tau' \in \Theta_{\text {good }}(\tau)\big]  \leq \sum_{i=1}^{6}\Pr[\bi]       \\
\leq  & 
\frac{w^3q^3}{N^2}+
\frac{2w^{2} q p^2}{N^{2}}+
\frac{w^{2} q p^2}{N^{2}}+
\frac{3w^2q(p+wq)^2}{N^2}+
\frac{2w^4q^2(p+wq)}{N^2}+
\frac{2w^4q^2(p+wq)}{N^2}             \\
\leq  & \frac{w^3q^3}{N^2} +
\frac{6w^2 q (p+wq)^2}{N^{2}} +
\frac{4w^4q^2(p+wq)}{N^2}.
\end{align*}
%
