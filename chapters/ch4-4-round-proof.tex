...
& +
\sum_{(x',a',b',y')\in\mathcal{Q}_C'} \underbrace{{\Pr}\big[(a\xor k_1)[i]=(a'\xor k_1)[i]\big]}_{\leq1/(N-p-wq)\ (\text{Lemma }\ref{lemma:coll-prob})}         \notag      \\
\leq  &  \frac{p}{N}+\frac{wq}{N}+\frac{q}{N-p-wq} \leq \frac{2(p+wq)}{N}.
\notag
\end{align}
where (\ref{eq:334}) follows from the fact that $k_1[j]$ and $k_1[j']$ are uniform and independent. Similarly, ${\Pr}\big[(b\xor T^{-1}(k_3))[j]\in V_3^{(1)}\big]\leq(p+wq)/N+q/(N-p-wq)$. Since we have at most $w^2q$ choices for $(x,a,b,y)$ and $i, j \in\{1, \ldots, w\}$, we have
%
$$
{\Pr}\big[\bsix\mid\neg\bthree\wedge\neg\bfour\big] \leq
