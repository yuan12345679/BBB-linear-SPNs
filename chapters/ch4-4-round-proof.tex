...
%	&~\Big|~S_t\vdash\mathcal{Q}_{S_t}^{(0)}\wedge(z\xor k)[i]\notin V_t^{(0)}\wedge(z\xor k)[i]\notin V_t^{(0)}\Big].    
	...
	\pcoll_{1}^+(t,z,z',j,j')\leq\frac{1}{N-p-wq}, \hugeindent
	\pcoll_{1}^-(t,z,z',j,j')\leq\frac{1}{N-p-wq}.    \notag
	%
...
\begin{proof}
	...
	By the above, in any case, the probability to have $T(\overline{S_t}(z\xor k_{t-1}))[j]=T(\overline{S_t}(z\xor k_{t-1}))[j]$ is at most $1/(N-p-wq)$, which establishes $\pcoll_{1}^+(t,z,z',j)\leq1/(N-p-wq)$. Similarly by symmetry, $\pcoll_{1}^-(t,z,z',j)\leq1/(N-p-wq)$.         \qed
%	
%	
%	\input{pcoll2.tex}
\end{proof}
...
Regarding \bfour, for any $(x,a,b,y),i,j$, we define six predicates as follows.
%
\begin{itemize}
	\item $\pred_1(x,a,i)$ is fulfilled, if $(a\xor k_1)[i]\in U_2^{(0)}$.
%	\begin{itemize}
%		\item $(a\xor k_1)[i]\in U_2^{(0)}$, or
%		\item \underline{there exists $(x',a',b',y')$ and $i'\neq i$ such that $(a\xor k_1)[i]=(a'\xor k_1)[i']$.}
%	\end{itemize}
	\item $\pred_2(x,a,i,i')$ is fulfilled, if there exists $(x',a',b',y')\neq(x,a,b,y)$ such that $a[i]=a'[i']$.
	\item $\pred_3(b,y,j)$ is fulfilled, if $(b\xor T^{-1}(k_3))[j]\in V_3^{(0)}$.
%	\begin{itemize}
%		\item $(b\xor T^{-1}(k_3))[j]\in V_3^{(0)}$, or
%		\item \underline{there exists $(x',a',b',y')$ and $j'\neq j$ such that} $(b\xor T^{-1}(k_3))[j]=(b'\xor T^{-1}(k_3))[j']$.
%	\end{itemize}
	\item $\pred_4(b,y,j,j')$ is fulfilled, if there exists $(x',a',b',y')\neq(x,a,b,y)$ such that $b[j]=b'[j']$.
	\item $\free_1(x,a)$ is fulfilled, if $(x\xor k_0)[i_0]\notin U_1^{(0)}$ for all $i_0\in\{1,\ldots,w\}$.
	\item $\free_2(b,y)$ is fulfilled, if $(b\xor T^{-1}(k_3))[i_0]\notin U_1^{(0)}$ for all $i_0\in\{1,\ldots,w\}$.
\end{itemize}
%
Since $k_1[i]$ and $k_1[i']$ are uniform and independent, it is easy to see $\Pr[\pred_1(x,a,i)]\leq(p+wq)/N$ and $\Pr[\pred_3(b,y,j)]\leq(p+wq)/N$.
...
\begin{align*}
&\Pr[\bfour]   \\
= &\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\Big[(a\xor k_1)[i]\in U_2^{(1)}\wedge(b\xor T^{-1}(k_3))[j]\in V_3^{(1)}\Big]       \\
= &\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\Big[\big(\pred_1(x,a,i)\vee\pred_2(x,a,i)\big)\wedge\big(\pred_3(b,y,j)\vee\pred_4(b,y,j)\big)\Big]        \\
= &\sum_{(x,a,b,y)}\sum_{i,j}\Big({\Pr}\big[\pred_1(x,a,i)\wedge\pred_3(b,y,j)\big]
+{\Pr}\big[\pred_1(x,a,i)\wedge\pred_4(b,y,j)\big]        \\
&\ \ \  +{\Pr}\big[\pred_2(x,a,i)\wedge\pred_3(b,y,j)\big]
+{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\big]\Big)       \\
\leq &\sum_{(x,a,b,y)}\sum_{i,j}\bigg(\frac{(p+wq)^2}{N^2}+
\frac{p+wq}{N}\cdot
{\Pr}\big[\pred_4(b,y,j)\big]      +\frac{p+wq}{N}\cdot
{\Pr}\big[\pred_2(x,a,i)\big]   \\
&\ \ \ \ \ \ \ \ \ \ \ \ 
+{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\big]\bigg)      \\
\leq &
%
\frac{w^2q(p+wq)^2}{N^2}+
\frac{p+wq}{N}\cdot
\sum_{(x,a,b,y)}\sum_{i,j}\Big(
{\Pr}\big[\pred_2(b,y,i)\big]     +
{\Pr}\big[\pred_4(x,a,j)\big]   \Big)   \\
&\ \ \ \ \ \ \ \ \ \ \ \ 
+
\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\big].
\end{align*}
%
%

For the term $\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\big[\pred_2(x,a,i)\big]$, we have
%
\begin{align*}
\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\big[\pred_2(x,a,i)\big]  
\leq &  \sum_{(x,a,b,y)}\sum_{i,j}
{\Pr}\big[\pred_2(x,a,i)\mid\free(x,a)\big]
        \\
%
&+\sum_{(x,a,b,y)}\sum_{i,j} \underbrace{{\Pr}\big[\pred_2(x,a,i)\mid\neg\free(x,a)\big]}_{\leq 1}
	\times \underbrace{{\Pr}\big[\neg\free(x,a)\big]}_{\leq wp/N}        \\
\leq &  \sum_{(x,a,b,y)}\sum_{i,j}
\sum_{(x',a',b',y')\neq(x,a,b,y)}\pcoll_{1}^+(t,x,x',i,i')
+\frac{w^3qp}{N}       \\
\leq &  \frac{w^2q^2}{N-p-wq}   +\frac{w^3qp}{N}    .
\end{align*}
%
Similarly, $\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\big[\pred_4(b,y,j)\big]  
\leq   \frac{w^2q^2}{N-p-wq}   +\frac{w^3qp}{N}    .$ Finally, for the term $\sum_{(x,a,b,y)}\sum_{i,j}{\Pr}[\pred_2(x,a,i)\wedge\pred_4(b,y,j)]$, we use
%
\begin{align*}
& \sum_{(x,a,b,y)}\sum_{i,j}{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\big]    \\
\leq &  \sum_{(x,a,b,y)}\sum_{i,j}
\underbrace{{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\mid\free_1(x,a)\wedge\free_2(x,a)\big]}_{\leq w^2q^2\times\pcoll_{1}^+(t,z,z',j,j')\times\pcoll_{1}^-(t,z,z',j,j')
\leq w^2q^2/(N-p-wq)^2}      \\
&+ \sum_{(x,a,b,y)}\sum_{i,j}
\underbrace{{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\wedge\neg\free_1(x,a)\wedge\free_2(x,a)\big]}_{\leq wq\times\pcoll_{1}^+(t,z,z',j,j')\times\Pr[\neg\free_1(x,a)]\leq w^2pq/N(N-p-wq)}      \\
&+ \sum_{(x,a,b,y)}\sum_{i,j}\underbrace{{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\wedge\free_1(x,a)\wedge\neg\free_2(x,a)\big]}_{\leq wq\times\pcoll_{1}^-(t,y,y',j,j')\times\Pr[\neg\free_2(x,a)]\leq w^2pq/N(N-p-wq)}      \\
&+ \sum_{(x,a,b,y)}\sum_{i,j}
\underbrace{{\Pr}\big[\pred_2(x,a,i)\wedge\pred_4(b,y,j)\wedge\neg\free_1(x,a)\wedge\neg\free_2(x,a)\big]}_{\leq{\Pr}[\neg\free_1(x,a)\wedge\neg\free_2(x,a)]\leq w^2p^2/N^2}       \\
\leq &  \frac{w^4q^3}{(N-p-wq)^2} +  \frac{2w^4pq^2}{N(N-p-wq)} + 
\frac{w^4p^2q}{N^2}   \leq    \frac{4w^4q^3+4w^4pq^2+w^4p^2q}{N^2}  .
\end{align*}
%
