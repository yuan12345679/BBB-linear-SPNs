...
\begin{lemma}
	...
	Define
	\begin{align*}
	&\pcoll_{1}^+(t,z,z',j,j')  :={\Pr}_{S_t}\Big[T\big(\overline{S_t}(z\xor k)\big)[j]=T\big(\overline{S_t}(z'\xor k)\big)[j']       \\
	&~\Big|~(z\xor k)[i]\notin U_t^{(0)}\Big],         \\
	&\pcoll_{2}^+(t,z,z',i,i',j,j')      :={\Pr}_{S_t}\Big[T\big(\overline{S_t}(z\xor k)\big)[j]=T\big(\overline{S_t}(z'\xor k)\big)[j']   \\
...
$\big(T\big(\overline{S_t}(z\xor k)\big)\xor k\big)[j]=\big(T\big(\overline{S_t}(z'\xor k)\big)\xor k\big)[j']$ is $1/N\leq 1/(N-p-wq)$, since $k[j]$ and $k[j']$ are uniform and independent. In the remaining we focus on the case of $j=j'$, which means $T\big(\overline{S_t}(z\xor k)\big)[j]=T\big(\overline{S_t}(z'\xor k)\big)[j]$. Note that $z\neq z'$ implies there exists $i_0$ such that $(z\xor k)[i_0]\neq(z'\xor k)[i_0]$. By the assumption, $(z'\xor k)[i_0]\notin U_1^{(0)}$. By construction, we have
%
\begin{align*}
...
Now:
\begin{itemize}
	\item If $x[\ell]=x'[\ell]$ for any $\ell\neq i$, then $v_1\neq v_1'$ by $z\neq z'$. In this case, Eq. (\ref{eq:interm-eq-b2}) collapses to $t_{j,i}\cdot v_1=t_{j,i}\cdot v_1'$ which is not possible since $t_{j,i}\neq 0$ and $v_1\neq v_1'$;
