...
\begin{itemize}[leftmargin=10mm]
	...
	\begin{itemize}
		...
		\item $(b\xor k_3)[i]=(b'\xor k_3)[i']=(b''\xor k_3)[i'']$, or
		\item $(y\xor k_4)[i]=(y'\xor k_4)[i']=(y''\xor k_4)[i'']$.
	\end{itemize}
	\item[\btwo] there exist $(x,a,b,y) \in \mathcal{Q}_{C}'$ and distinct $i, i' \in \{1, \ldots, w\}$ such that:
	\begin{itemize}
	...
	\end{itemize}
	\item[\bthree] there exist $(x,a,b,y) \in \mathcal{Q}_{C}', u_1 \in U_1^{(0)}, v_4 \in V_4^{(0)}$, and index $i, j \in \{1, \ldots, w\}$ such that $\left(x \oplus k_{0}\right)[i]=u_1$ and $\left(y \oplus k_{4}\right)[j]=v_4$.
	\item[\bfour] there exist $(x,a,b,y) \in \mathcal{Q}_{C}', u_1 \in U_1^{(0)}, u_2\in U_2^{(1)}$, and index $i, j \in \{1, \ldots, w\}$ such that $\left(x \oplus k_{0}\right)[i]=u_1$ and $\left(a\oplus k_{1}\right)[j]=u_2$.
%	there exist $(x,a,b,y) \in \mathcal{Q}_{C}', u_1 \in U_1^{(0)}, u_2\in U_2^{(0)}$, and index $i, j \in \{1, \ldots, w\}$ such that $\left(x \oplus k_{0}\right)[i]=u_1$ and $\left(T\left(\overline{S_1}\left(x \oplus k_{0}\right) \oplus k_{1}\right)\right)[j]=u_2$.
	\item[\bfive] there exist $(x,a,b,y) \in \mathcal{Q}_{C}', v_{3}\in V_3^{(1)}, v_{4}\in V_4^{(0)}$, and index $i, j \in \{1, \ldots, w\}$ such that $\left(y \oplus k_{4}\right)[j]=v_4$ and $\left(b\xor T^{-1}(k_3)\right) [i]=v_3$.
...
%	there exist $(x,a,b,y) \in \mathcal{Q}_{C}'$ and $i, j \in\{1, \ldots, w\}$ such that $(a \oplus k_1)[i]\in U_{2}^{(1)}$ and $(T(\overline{S_2}(a \oplus k_1))\xor k_2)[j]\in U_{3}^{(1)}$.
	...
	\item[\bseven]
	there exist $(x,a,b,y) \in \mathcal{Q}_{C}'$, $i, j\in\{1, \ldots, w\}$ such that:
	\begin{itemize}
		\item $(a \oplus k_1)[i]\in U_{2}^{(1)}$ and $(T(\overline{S_2}(a \oplus k_1))\xor k_2)[j]\in U_{3}^{(1)}$, or
		\item $(T^{-1}(\overline{S_3^{-1}}(b\xor T^{-1}(k_3))\xor k_2))[i]\in U_2^{(1)}$ and $(b\xor T^{-1}(k_3))[j]\in V_{3}^{(1)}$.
	\end{itemize}
...
\begin{lemma}
	...
	Define
	\begin{align*}
	&\pcoll_{1}^+(t,z,z',j)  :={\Pr}_{S_t}\Big[T\big(\overline{S_t}(z\xor k)\big)[j]=T\big(\overline{S_t}(z'\xor k)\big)[j]       \\
	&~\Big|~(z\xor k)[i]\notin U_t^{(0)}\Big],         \\
	&\pcoll_{2}^+(t,z,z',i,i',j)      :={\Pr}_{S_t}\Big[T\big(\overline{S_t}(z\xor k)\big)[j]=T\big(\overline{S_t}(z'\xor k)\big)[j]   \\
	&~\Big|~(z\xor k)[i]\notin U_t^{(0)}\wedge(z\xor k)[i]\notin U_t^{(0)}\Big],         \\
	&\pcoll_{1}^-(t,z,z',j)     :={\Pr}_{S_t}\Big[T\big(\overline{S_t^{-1}}(z\xor k)\big)[j]=T\big(\overline{S_t^{-1}}(z'\xor k)\big)[j]    \\
	&~\Big|~(z\xor k)[i]\notin V_t^{(0)}\Big],         \\
	&\pcoll_{2}^-(t,z,z',i,i',j)    :={\Pr}_{S_t}\Big[T\big(\overline{S_t^{-1}}(z\xor k_{t-1})\big)[j]=T\big(\overline{S_t^{-1}}(z'\xor k_{t-1})\big)[j]     \\
	&~\Big|~(z\xor k)[i]\notin V_t^{(0)}\wedge(z\xor k)[i]\notin V_t^{(0)}\Big].    
%	\label{eq:defn-pcoll}
	\end{align*}
	Then it holds
	\begin{align}
	{\Pr}_{S_t}\Big[T\big(\overline{S_t}(z\xor k)\big)[j]=\delta ~\Big|~\forall\ell\in\{1,\ldots,w\}:(z\xor k)[\ell]\notin U_t^{(0)}\Big]\leq\frac{1}{N},        \notag   \\
...
Then, consider $\pcoll_{1}^+(t,z,z',j)$. Note that $z\neq z'$ implies there exists $i_0$ such that $(z\xor k)[i_0]\neq(z'\xor k)[i_0]$. By the assumption, $(z'\xor k)[i_0]\notin U_1^{(0)}$. By construction, we have
%
\begin{align*}
&  T(\overline{S_1}(z\xor k_0))[j]\xor T(\overline{S_1}(z'\xor k_0))[j]       \\
= &
\Big(\bigoplus_{1\leq\ell\leq w}t_{j,\ell}\cdot S_1\big((z\xor k_0)[\ell]\big)\Big)\xor
\Big(\bigoplus_{1\leq\ell\leq w}t_{j,\ell}\cdot S_1\big((z'\xor k_0)[\ell]\big)\Big)   .
\end{align*}
%
...
\subsubsection{Case 1: $(x'\xor k_0)[i_0]$ is ``unique'',}
...
\subsubsection{Case 2: $(x'\xor k_0)[i_0]=(x'\xor k_0)[i_1]$ for some $i_1\neq i_0$.}
...
this case is similar to Case 2 by symmetry.
...
By the above, in any case, the probability to have $T(\overline{S_1}(x\xor k_0))[j]=T(\overline{S_1}(x'\xor k_0))[j']$ is at most $1/(N-p-wq)$.
...
The analysis of $\pcoll_{2}^+(t,z,z',i,i',j)$



First, consider $\pcoll_{2}^+(t,z,z',i,i',j)$ with $i\neq i'$. Since $z\neq z'$, there exists $i_0$ such that $(z\xor k_0)[i_0]\neq(z'\xor k_0)[i_0]$. Then either $i\neq i_0$ or $i'\neq i_0$. Wlog assume $i\neq i_0$. Note that this means $(x'\xor k_0)[i]\neq(x\xor k_0)[i_0]$, as otherwise both $(x'\xor k_0)[i]$ and $(x\xor k_0)[i_0]$ fall in $U_1^{(0)}$ and it contradicts $\neg\btwo$. The remaining discussion resembles the previous one for $\pcoll_{1}^+(t,z,z',j)$, which consists of 3 cases shown in Appendix XXX. In all, the uniformness of the value $S_1((x\xor k_0)[i_0])$ is sufficient to ensure that ${\Pr}[T(\overline{S_1}(x\xor k_0))[j]\xor T(\overline{S_1}(x'\xor k_0))[j]]\leq\frac{1}{N-p-wq}$.



Then, consider $\pcoll_{2}^+(t,z,z',i,i,j)$. Assume that $S_1((x\xor k_0)[i])=u_1$ and $S_1((x'\xor k_0)[i])=u_1'$ for $(u_1,v_1),(u_1',v_1')\in\mathcal{Q}_{S_1}^{(0)}$. Then it holds      {\small
	%
	\begin{align}
		&  T(\overline{S_1}(x\xor k_0))[j]\xor T(\overline{S_1}(x'\xor k_0))[j]       \notag   \\
		= &
...
\end{itemize}
Therefore, in this case, it still holds $\pcoll_2^+(t,z,z',i,i,j)\leq1/(N-p-wq)$.         \qed
\end{proof}
...
\begin{itemize}
	...
	\item(B-42) there exists $(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'$, and $i,j,j'\in\{1,\ldots,w\}$ such that $j\neq j'$, while $\left(x \oplus k_{0}\right)[i]\in U_1^{(0)}$ and $(a\xor k_1)[j]=(a'\xor k_1)[j']$;
	\item(B-43) there exists $(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'$, and $i,j\in\{1,\ldots,w\}$ such that $x\neq x'$, $(x'\xor k_0)[i']\notin U_1^{(0)}$ for all $i'\in\{1,\ldots,w\}$, while $\left(x \oplus k_{0}\right)[i]\in U_1^{(0)}$ and $(a\xor k_1)[j]=(a'\xor k_1)[j]$.
	\item(B-44) there exists $(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'$, and $i,i',j\in\{1,\ldots,w\}$ such that $x\neq x'$, while $\left(x \oplus k_{0}\right)[i]\in U_1^{(0)}$, $\left(x' \oplus k_{0}\right)[i']\in U_1^{(0)}$, and $(a\xor k_1)[j]=(a'\xor k_1)[j]$.
\end{itemize}
...
For (B-43), we have $\pcoll_{1}^+(1,x,x',j)\leq1/(N-p-wq)$ by Lemma \ref{lemma:coll-prob}. Thus
%
\begin{align*}
...
\noindent \textsc{\bsix}. For any construction query $(x,a,b,y)\in\mathcal{Q}_C'$, to have $(a\xor k_1)[i]\in U_2^{(1)}$, it has to be $(x\xor k_0)[i_0]\notin U_1^{(0)}$ for any $i_0\in\{1,\ldots,w\}$, as otherwise it contradicts $\neg\bfour$. Therefore,
%
%\begin{align*}
...
\noindent \textsc{\bseven}. Following the analysis of \cone, for any $(x,a,b,y)\in\mathcal{Q}_C'$, the probability to have $(a\xor k_1)[i]\in U_{2}^{(1)}$ is at most $(p+wq)/(N-p-wq)$. On the other hand, since $k_{2}$ is uniform and independent from the queries and from $k_{0}$, $k_{1}$, the probability to have $(T(\overline{S_2}(a \oplus k_1))\xor k_2)[j]\in U_{3}^{(1)}$ is $1/N$. Since we have at most $qw^2$ choices for $(x,a,b,y)$ and $i, j \in\{1, \ldots, w\}$, we have
%
%
$$
...
\noindent\textsc{\beight and \bnine}. Consider \beight first. The argument slightly resembles that of \bfour. In detail, \beight consists of two subevents as follows:
%
