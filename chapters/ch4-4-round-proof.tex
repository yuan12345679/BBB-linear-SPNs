...
\begin{itemize}[leftmargin=10mm]
	\item[\bone] there exists $(x,a,b,y) \in \mathcal{Q}_{C}'$ and distinct indices $i, i' \in \{1, \ldots, w\}$ such that $(x\xor k_0)[i]\in U_1^{(0)}\wedge(x\xor k_0)[i']\in U_1^{(0)}$, or $(y\xor k_4)[i]\in V_4^{(0)}\wedge(y\xor k_4)[i']\in V_4^{(0)}$.
	\item[\btwo]
...
%	there exists $(x,a,b,y) \in \mathcal{Q}_{C}', v_{3}\in V_3^{(0)}, v_{4}\in V_4^{(0)}$, and index $i, j \in \{1, \ldots, w\}$ such that $\left(y \oplus k_{4}\right)[j]=v_4$ and $\left(T^{-1}\left(\overline{S_4^{-1}}\left(y \oplus k_{4}\right)\right)\xor T^{-1}(k_3)\right) [i]=v_3$.
	\item[\bfour] \textcolor{red}{there exists $(x,a,b,y) \in \mathcal{Q}_{C}'$ and distinct indices $i, j \in \{1, \ldots, w\}$ such that $(x\xor k_0)[i]=(x\xor k_0)[j]$, or $(y\xor k_4)[i]=(y\xor k_4)[j]$.}
\end{itemize}
%
%
...
\noindent \textsc{\bone}.
%
For each of the $w^{2}qp^2$ choices of $(x,a,b, y) \in \mathcal{Q}_{C}', \left(u_{1}, v_{1}\right) \in \mathcal{Q}_{S_{1}}^{(0)}, \left(u_{4}, v_{4}\right) \in \mathcal{Q}_{S_{4}}^{(0)}$ and indices $i, j \in \{1, \ldots, w\}$, since $k_{0}$ and $k_{4}$ are uniform and independent, the probability to have $(x \oplus k_{0})[i]=u_1$ and $(y \oplus k_{4})[j]=v_4$ is $1/N^2$. Thus
%
$$
...
\noindent \textsc{\btwo, and \bthree}.
%
Note that \btwo could be divided into three subevents:
\begin{itemize}
	...
	\item(B-32) there exists $(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'$, $(u_1,v_1)\in\mathcal{Q}_{S_1}^{(0)}$, and $i,j,j'\in\{1,\ldots,w\}$ such that $(a,j)\neq(a',j')$, $(x'\xor k_0)[i']\notin U_1^{(0)}$ for all $i'\in\{1,\ldots,w\}$, while $\left(x \oplus k_{0}\right)[i]=u_1$ and $(a\xor k_1)[j]=(a'\xor k_1)[j']$.
	\item(B-33) there exists $(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'$, $(u_1,v_1),(u_1',v_1')\in\mathcal{Q}_{S_1}^{(0)}$, and $i,i',j,j'\in\{1,\ldots,w\}$ such that $(a,j)\neq(a',j')$, while $\left(x \oplus k_{0}\right)[i]=u_1$, $\left(x' \oplus k_{0}\right)[i']=u_1'$, and $(a\xor k_1)[j]=(a'\xor k_1)[j']$.
\end{itemize}
It is easy to see $\Pr[\text{(B-31)}]\leq w^2qp^2/N^2$: since $k_0$ and $k_1$ are uniform and independent, it holds ${\Pr}\big[(x\xor k_0)[i]\in U_1^{(0)}\wedge(a\xor k_1)[j]\in U_2^{(0)}\big]=1/N^2$.



For (B-32), note that there exists $i_0$ such that $(x\xor k_0)[i_0]\neq(x'\xor k_0)[i_0]$. By the assumption, $(x'\xor k_0)[i_0]\notin U_1^{(0)}$. Therefore, conditioned on $S_1\vdash\mathcal{Q}_{S_1}^{(0)}$, the value of $S_1((x'\xor k_0)[i_0])$ remains uniform in at least $N-p-wq$ possibilities. Because every entry in the $i_0$th column of $T$ is nonzero, we have
%
$${\Pr}\big[T(\overline{S_1}(x\xor k_0))[j]\xor T(\overline{S_1}(x'\xor k_0))[j']=k_1[j]\xor k_1[j']\big]\leq\frac{1}{N-p-wq}.$$
%
Meanwhile, it remains $\Pr[\left(x \oplus k_{0}\right)[i]=u_1]=1/N$. Therefore, $\Pr[\text{(B-32)}]\leq w^3q^2p/N(N-p-wq)$.




For (B-33), we derive the probability as follows.
%
\begin{align*}
\Pr[\text{(B-33)}]  
=   &  \sum_{(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'}\sum_{i,i',j,j'}\underbrace{\Pr[(x\xor k_0)[i]\in U_1^{(0)}]}_{\leq p/N}     \\
   & \hugeindent\times
	\underbrace{\Pr[(x'\xor k_0)[i]\in U_1^{(0)}|(x\xor k_0)[i]\in U_1^{(0)}]}_{\leq1}        \\
   & \hugeindent\times\Pr\big[\underbrace{(a\xor k_1)[j]=(a'\xor k_1)[j']|   
   (x'\xor k_0)[i]\in U_1^{(0)}\wedge(x\xor k_0)[i]\in U_1^{(0)}}_{\coll(x,x',i,i',j,j')}\big]       \\
\leq  &  \frac{p}{N}\cdot\bigg(
\underbrace{\sum_{(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'}\sum_{i,i'}\sum_{j\neq j'}\Pr[\coll(x,x',i,i',j,j')]}_{A_1}     \\
& \hugeindent\hugeindent   +  
\underbrace{\sum_{(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'}\sum_{i\neq i'}\sum_{j}\Pr[\coll(x,x',i,i',j,j)]}_{A_2}      \\ 
& \hugeindent\hugeindent   +  \underbrace{\sum_{(x,a,b,y),(x',a',b',y')\in\mathcal{Q}_{C}'}\sum_{i}\sum_{j}\Pr[\coll(x,x',i,i,j,j)]}_{A_3}\bigg)     .
...
When $j\neq j'$, we have $\Pr[\coll(x,x',i,i',j,j')]=1/N$ by that $k_1[j]$ and $k_1[j']$ are uniform and independent. Therefore, $A_1\leq w^4q^2p/N^2$.


Next, consider $\Pr[\coll(x,x',i,i',j,j)]$ with $i\neq i'$. Since $x\neq x'$, there exists $i_0$ such that $(x\xor k_0)[i_0]\neq(x'\xor k_0)[i_0]$. Then either $i\neq i_0$ or $i'\neq i_0$. Wlog assume $i\neq i_0$. Then by $\neg\bone$, $S_1((x\xor k_0)[i_0])$ remains uniform in at least $N-p-wq$ possibilities. Because every entry in the $i_0$th column of $T$ is nonzero, we have ${\Pr}[T(\overline{S_1}(x\xor k_0))[j]\xor T(\overline{S_1}(x'\xor k_0))[j]]\leq\frac{1}{N-p-wq}$, which means $A_2\leq w^3q^2p/N(N-p-wq)$.



Finally, consider $\Pr[\coll(x,x',i,i,j,j)]$. Assume that $\overline{S_1}(x\xor k_0)=\bfv_1\|v_1\|\bfv_2$ and
...
\end{itemize}
Therefore, in this case, it still holds $\Pr[\coll(x,x',i,i,j,j)]\leq\frac{1}{N-p-wq}$, which means $A_3\leq w^2q^2p/N(N-p-wq)$.


Summing over the above, we reach
%
$$\Pr[\btwo]\leq\frac{4w^3q^2p}{N(N-p-wq)}.$$
%
Similarly, $\operatorname{Pr}\left[\bthree\right] \leq \frac{w^{2} q \left(p+w q\right)^{2}}{N^{2}}$ by symmetry.
